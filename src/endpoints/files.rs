//! Entry point for interacting with the Xero Files API.

use crate::auth::TokenSet;
use crate::client::XeroClient;
use crate::error::XeroError;
use crate::http::ApiClient;
use crate::models::files::{
    association::{Association, AssociationCount, AssociationsResponse},
    file::{File, FilesResponse},
    folder::{Folder, FoldersResponse},
};
use reqwest::{multipart, Method};
use serde::Serialize;
use std::sync::Arc;
use uuid::Uuid;

const BASE_URL: &str = "https://api.xero.com/files.xro/1.0";

/// A handle to the Files API endpoints.
#[derive(Debug, Clone)]
pub struct FilesApi {
    client: ApiClient,
}

impl FilesApi {
    pub(crate) fn new(client: XeroClient, tenant_id: Uuid) -> Self {
        Self {
            client: ApiClient::new(
                BASE_URL,
                tenant_id,
                client.http_client.clone(),
                client.token_manager.clone(),
                client.rate_limiter.clone(),
            ),
        }
    }

    pub(crate) fn with_token_override(mut self, token: Arc<TokenSet>) -> Self {
        self.client = self.client.with_token_override(token);
        self
    }

    // --- Files ---
    /// Retrieves a list of files.
    pub async fn get_files(
        &self,
        page_size: Option<u32>,
        page: Option<u32>,
        sort: Option<String>,
        direction: Option<String>,
    ) -> Result<Vec<File>, XeroError> {
        let mut query = Vec::new();
        if let Some(ps) = page_size {
            query.push(("pagesize".to_string(), ps.to_string()));
        }
        if let Some(p) = page {
            query.push(("page".to_string(), p.to_string()));
        }
        if let Some(s) = sort {
            query.push(("sort".to_string(), s));
        }
        if let Some(d) = direction {
            query.push(("direction".to_string(), d));
        }
        let resp: FilesResponse = self
            .client
            .send_request(Method::GET, "/Files", Some(&query), None::<()>)
            .await?;
        Ok(resp.items)
    }

    /// Retrieves a specific file by its ID.
    pub async fn get_file_by_id(&self, file_id: Uuid) -> Result<File, XeroError> {
        let path = format!("/Files/{file_id}");
        self.client
            .send_request(Method::GET, &path, None, None::<()>)
            .await
    }

    /// Downloads the content of a specific file.
    pub async fn get_file_content(&self, file_id: Uuid) -> Result<Vec<u8>, XeroError> {
        let path = format!("/Files/{file_id}/Content");
        self.client
            .send_request_bytes(Method::GET, &path, None)
            .await
    }

    async fn upload_file_internal(
        &self,
        path: &str,
        file_name: String,
        body: Vec<u8>,
    ) -> Result<File, XeroError> {
        let part = multipart::Part::bytes(body).file_name(file_name.clone());
        let form = multipart::Form::new().part("file", part);
        self.client
            .send_request_multipart(Method::POST, path, form)
            .await
    }

    /// Uploads a file to the inbox.
    pub async fn upload_file(&self, file_name: String, body: Vec<u8>) -> Result<File, XeroError> {
        self.upload_file_internal("/Files", file_name, body).await
    }

    /// Uploads a file to a specific folder.
    pub async fn upload_file_to_folder(
        &self,
        folder_id: Uuid,
        file_name: String,
        body: Vec<u8>,
    ) -> Result<File, XeroError> {
        let path = format!("/Files/{folder_id}");
        self.upload_file_internal(&path, file_name, body).await
    }

    /// Updates a file's name or folder.
    pub async fn update_file(
        &self,
        file_id: Uuid,
        new_name: Option<String>,
        new_folder_id: Option<Uuid>,
    ) -> Result<File, XeroError> {
        #[derive(Serialize)]
        #[serde(rename_all = "PascalCase")]
        struct UpdateFileRequest {
            #[serde(skip_serializing_if = "Option::is_none")]
            name: Option<String>,
            #[serde(rename = "FolderId", skip_serializing_if = "Option::is_none")]
            folder_id: Option<Uuid>,
        }
        let body = UpdateFileRequest {
            name: new_name,
            folder_id: new_folder_id,
        };
        let path = format!("/Files/{file_id}");
        self.client
            .send_request(Method::PUT, &path, None, Some(body))
            .await
    }

    /// Deletes a file.
    pub async fn delete_file(&self, file_id: Uuid) -> Result<(), XeroError> {
        let path = format!("/Files/{file_id}");
        self.client
            .send_request_empty_response(Method::DELETE, &path, None::<()>)
            .await
    }

    // --- Folders ---
    /// Retrieves a list of all folders.
    pub async fn get_folders(&self, sort: Option<String>) -> Result<Vec<Folder>, XeroError> {
        let mut query = Vec::new();
        if let Some(s) = sort {
            query.push(("sort".to_string(), s));
        }
        let resp: FoldersResponse = self
            .client
            .send_request(Method::GET, "/Folders", Some(&query), None::<()>)
            .await?;
        Ok(resp.into_vec())
    }

    /// Retrieves a specific folder by its ID.
    pub async fn get_folder_by_id(&self, folder_id: Uuid) -> Result<Folder, XeroError> {
        let path = format!("/Folders/{folder_id}");
        self.client
            .send_request(Method::GET, &path, None, None::<()>)
            .await
    }

    /// Creates a new folder.
    pub async fn create_folder(&self, name: String) -> Result<Folder, XeroError> {
        #[derive(Serialize)]
        struct CreateFolderRequest {
            name: String,
        }
        let body = CreateFolderRequest { name };
        self.client
            .send_request(Method::POST, "/Folders", None, Some(body))
            .await
    }

    /// Updates a folder's name.
    pub async fn update_folder(&self, folder_id: Uuid, name: String) -> Result<Folder, XeroError> {
        #[derive(Serialize)]
        struct UpdateFolderRequest {
            name: String,
        }
        let body = UpdateFolderRequest { name };
        let path = format!("/Folders/{folder_id}");
        self.client
            .send_request(Method::PUT, &path, None, Some(body))
            .await
    }

    /// Deletes a folder.
    pub async fn delete_folder(&self, folder_id: Uuid) -> Result<(), XeroError> {
        let path = format!("/Folders/{folder_id}");
        self.client
            .send_request_empty_response(Method::DELETE, &path, None::<()>)
            .await
    }

    // --- Associations ---
    /// Retrieves a list of associations for a particular file.
    pub async fn get_file_associations(
        &self,
        file_id: Uuid,
    ) -> Result<Vec<Association>, XeroError> {
        let path = format!("/Files/{file_id}/Associations");
        let resp: AssociationsResponse = self
            .client
            .send_request(Method::GET, &path, None, None::<()>)
            .await?;
        Ok(resp.into_vec())
    }

    /// Retrieves a list of associations for a particular object (e.g., an invoice).
    pub async fn get_object_associations(
        &self,
        object_id: Uuid,
    ) -> Result<Vec<Association>, XeroError> {
        let path = format!("/Associations/{object_id}");
        let resp: AssociationsResponse = self
            .client
            .send_request(Method::GET, &path, None, None::<()>)
            .await?;
        Ok(resp.into_vec())
    }

    /// Retrieves a count of associations for a list of objects.
    pub async fn get_associations_count(
        &self,
        object_ids: Vec<Uuid>,
    ) -> Result<AssociationCount, XeroError> {
        let ids_str = object_ids
            .iter()
            .map(std::string::ToString::to_string)
            .collect::<Vec<String>>()
            .join(",");
        let query = vec![("ObjectIds".to_string(), ids_str)];
        self.client
            .send_request(Method::GET, "/Associations/Count", Some(&query), None::<()>)
            .await
    }

    /// Creates an association between a file and an object.
    pub async fn create_association(
        &self,
        file_id: Uuid,
        association: Association,
    ) -> Result<Association, XeroError> {
        let path = format!("/Files/{file_id}/Associations");
        self.client
            .send_request(Method::POST, &path, None, Some(association))
            .await
    }

    /// Deletes an association.
    pub async fn delete_association(
        &self,
        file_id: Uuid,
        association_id: Uuid,
    ) -> Result<(), XeroError> {
        let path = format!("/Files/{file_id}/Associations/{association_id}");
        self.client
            .send_request_empty_response(Method::DELETE, &path, None::<()>)
            .await
    }
}
