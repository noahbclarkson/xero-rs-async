...
use crate::models::payroll_nz::timesheet;
use reqwest::Method;
use uuid::Uuid;
...
impl<'a> TimesheetsResource<'a> {
    ...
    ) -> Result<timesheet::TimesheetLine, XeroError> {
        let path = format!("/timesheets/{timesheet_id}/lines/{line_id}");
        let resp: timesheet::TimesheetLineResponse = self
            .api
            .client
            .send_request(Method::PUT, &path, None, Some(line))
            .await?;
        resp.timesheet_line.ok_or(XeroError::Api {
            status: reqwest::StatusCode::NOT_FOUND,
            message: "Timesheet line not found in response".to_string(),
        })
    }
